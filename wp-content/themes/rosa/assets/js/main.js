...
$(window).load(function(){
    ...
    if(!empty($('#date-otreservations'))){
		var dateFormat = $('#date-otreservations' ).closest('.otw-wrapper' ).children('.txtDateFormat' ).attr('value' );
...
jQuery(document).ready(function(){
	jQuery(".otreservations-submit").unbind('click').click(function(e){
		e.preventDefault();debugger
		var $date = jQuery('#date-otreservationsFix').val();
		var $time = jQuery('#time-otreservations').val();
		var $people = jQuery('#party-otreservations').val();
		var selected_year = parseInt($date.split("-")[0]);
		var selected_month = parseInt($date.split("-")[1]);
		var selected_day = parseInt($date.split("-")[2]);
		...
		if($date == "" || $date.length == 0){
			alert("Seleccione una fecha para la reservación!");
			jQuery('#date-otreservationsFix').focus();
			return;
		}else if(selected_year > year || selected_year < year){
			alert("Seleccione una fecha en este año!");
			jQuery('#date-otreservationsFix').focus();
			return;
		}else if((selected_month - 1) < month){
			alert("Seleccione una fecha que sea en este mes o mas adelante!");
			jQuery('#date-otreservationsFix').focus();
			return;
		}else{
			if((selected_month - 1) == month){
				if(selected_day < day){
					alert("Seleccione una fecha que sea mayor a este dia!");
					jQuery('#date-otreservationsFix').focus();
					return;
				}else{
			
					jQuery(".pixcode--otreservations").fadeOut("500",function(){
						jQuery(".revervation-form.back-form").fadeIn("500")	;
					});
					
					jQuery("#btn_enviar").unbind("click").click(function(e){
						e.preventDefault();
						...
						if($name == "" || $email == "" || $phone == "" || $observations == ""){
							...
							alert("El correo electronico no es valido, verificalo nuevamente!");
							jQuery('#email').focus();
						}else if(isNaN($phone)){
							alert("El número de telefono no es correcto!");
							jQuery('#phone').focus();
						}else{
							// ajax que envia los datos al correo electronico
							jQuery.ajax({
								...
								success	: function(res){
									...
									if(res == "1"){
										alert("La reservación se envio correctamente!");
										
										jQuery("#name").val("");
										jQuery("#email").val("");
										jQuery("#phone").val("");
										jQuery("#observations").val("");
										jQuery('#date-otreservations').val("");
										
										jQuery(".revervation-form.back-form").fadeOut("500",function(){
											jQuery(".pixcode--otreservations").fadeIn("500");
										});
									}else{
